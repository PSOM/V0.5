...
MODULE particles
  USE header, ONLY : NI,NJ,NK,uf,vf,wf,Jifc,Jjfc,J2d,ux,vy,NPR,wz,PI,dtf,vor,pv,shear,rho,strain,zf,&
...
CONTAINS
  ...
  SUBROUTINE ini_particles(time)
    ...
    DO i=1, NPR
      i1=MOD(i,100)
      parti(i)%i=REAL(i1*REAL(NI)/100.)
      parti(i)%j=81.+REAL(i-i1)/100.
      parti(i)%k=2.
      !PRINT*,i,parti(i)%i,parti(i)%j,parti(i)%k
    ENDDO
  ...
  SUBROUTINE get_parti_vel(time)
    ...
    DO ip = 1, NPR
       ...
       IF (parti(ip)%j < NJ .AND. parti(ip)%j > 0 .AND. &
          ...
          CALL interp_trilinear(dic,djc,dz,pv(ic:ic+1,jc:jc+1,kc:kc+1),parti(ip)%shear)
