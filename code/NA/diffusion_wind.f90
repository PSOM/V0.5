...
!    
      ...
      integer i,j,k,m,step,nday,nj1,nj2
...
!     Use the same value for mixing tracers and momentum
     ...
     &     vdif(NI,NJ,NK),Kdudzb,Kdvdzb,Kdudzt,Kdvdzt,wzkth,fact,Cp
      double precision RR,fac,facb,dfactor,Kzmax,facy,rhoinv,diff,
     &     stressy,wgt,day,ts,ycenter, ymax,KzmaxTr,
     &     ztransit,zextent,thy
      double precision rhoflux(NI,NJ)
      parameter (nj1=36, nj2=157)  !for NJ=192
...
!      Kzmax= 1.d-2
      Kzmax= dmax1(1.d-5, 1.d-2* (dabs(stressmax) / 0.2d0))
      KzmaxTr= Kzmax
...
!-     &        dtanh( pi*dble(j-nj1/2)/dble(nj1/2)) )
!-      end do
!-      do j=nj1+1,nj2
!-         stressx(j)= stressmax * 1.d0
!-      end do
!-      dj= 0.5d0*dble(NJ-nj2)
!-      dmid= dble(nj2)+dj
!-      do j=nj2+1,NJ
!-         stressx(j)= stressmax* 0.5*(1.-
...
!     --------------------------------
 101   continue
      ...
      do j=1,NJ/2
         yc(j)= dble(j)-0.5
         stressx(j) = stressmax* 0.5*(1.d0 +tanh((( yc(j)-yset)/yw)*PI ))
      end do
...
!     Assumes that rho is evaluated just prior to this call
      ...
      integer n1
      parameter (n1=3)
      double precision fac,bvfreq,grho,RiCr,Ri,shear,DLinv
...
!     BVfreq is N**2 and is in s^-2 if re-dim by DLinv
         shear= (dudz(k)*dudz(k) + dvdz(k)*dvdz(k))*fac
         if (shear.eq.0.d0) then
